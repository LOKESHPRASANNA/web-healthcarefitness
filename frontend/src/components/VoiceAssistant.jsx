    ...
    <>
      ...
      <AnimatePresence>
        {showHUD && (
          ...
          >
            <button onClick={() => setShowHUD(false)} className="absolute top-3 right-3 text-slate-400 hover:text-white">
              <X size={16} />
            </button>
            
            <div className="relative mb-4">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center bg-slate-900 border-2 ${isListening ? 'border-accent shadow-[0_0_20px_rgba(59,130,246,0.5)]' : 'border-slate-700'}`}>
                {isListening ? (
            ...
            <div className="h-16 flex flex-col justify-center">
              {isListening ? (
                ...
                <>
                  <p className="text-slate-400 text-sm italic mb-2">"{transcript}"</p>
      ...
      <motion.button 
        ...
        className={`fixed bottom-8 right-8 z-50 w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-colors ${
          isListening 
            ? 'bg-red-500 hover:bg-red-600 text-white shadow-[0_0_20px_rgba(239,68,68,0.4)]' 
            : 'bg-accent hover:bg-blue-600 text-white shadow-[0_0_20px_rgba(59,130,246,0.4)]'
        }`}
      >
