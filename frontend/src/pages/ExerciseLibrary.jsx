...
const library = [
  { id: 1, name: 'Barbell Back Squat', category: 'Legs', difficulty: 'Hard', equipment: 'Barbell', muscles: 'Quads, Glutes', image: 'https://images.unsplash.com/photo-1581009146145-14e5300c3a48?q=80&w=400&auto=format&fit=crop' },
  { id: 2, name: 'Conventional Deadlift', category: 'Back', difficulty: 'Expert', equipment: 'Barbell', muscles: 'Hamstrings, Lower Back', image: 'https://images.unsplash.com/photo-1574680096145-d05b474e2155-d25dfeac3438?q=80&w=400&auto=format&fit=crop' },
  { id: 3, name: 'Dumbbell Bench Press', category: 'Chest', difficulty: 'Medium', equipment: 'Dumbbell', muscles: 'Pecs, Triceps', image: 'https://images.unsplash.com/photo-1581009146145-b5ef050c2e1e?q=80&w=400&auto=format&fit=crop' },
    ...
    <div className="max-w-7xl mx-auto h-full pb-20 relative">
      ...
      >
        <div>
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-white tracking-tight mb-2">Exercise Library</h1>
          <p className="text-slate-400">Browse movements, view 3D muscle targeting, and watch AR-ready tutorials.</p>
        </div>
      </motion.div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
        <div className="flex bg-slate-900/60 p-1.5 rounded-2xl border border-white/5 overflow-x-auto w-full md:w-auto scrollbar-hide">
          {categories.map(cat => (
            <button 
              ...
              className={`px-5 py-2.5 rounded-xl font-bold text-sm whitespace-nowrap transition-all ${activeCategory === cat ? 'bg-cyan-500 text-white shadow-lg shadow-cyan-500/25' : 'text-slate-400 hover:text-white'}`}
            >
              {cat}
            </button>
          ))}
        ...
        <div className="relative w-full md:w-72">
          ...
          <input 
            ...
            className="w-full bg-slate-900/60 border border-white/10 rounded-xl pl-12 pr-4 py-3 text-white outline-none focus:border-cyan-500 transition-colors text-sm"
          />
        </div>
      </div>
        ...
        {filteredExercises.map((exercise, index) => (
          ...
          >
            ...
            <div className="p-5 flex-1 flex flex-col">
              ...
              <h3 className="text-lg font-bold text-white mb-4 line-clamp-1">{exercise.name}</h3>
              ...
              <div className="space-y-2 mb-6 text-sm text-slate-300">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-white">{exercise.muscles}</span>
                </div>
              </div>

              <div className="mt-auto grid grid-cols-2 gap-3 border-t border-white/5 pt-4">
                <button className="flex items-center justify-center gap-2 text-sm font-bold text-white bg-slate-800 hover:bg-slate-700 py-2 rounded-xl transition-colors border border-white/5">
